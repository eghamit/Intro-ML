...
\begin{document}
	...
	Squared distance is enough to rank neighbours ($d^2=(6-x)^2+(5-y)^2$). Sorting
	all fifteen points (nearest first); the \textbf{seven nearest} are in bold:
	\begin{center}
	\small
	\renewcommand{\arraystretch}{1.15}
	\begin{tabular}{c c c c c}
	\toprule
	Rank & Point & Class & $d^2$ & $d$\\
	\midrule
	\textbf{1}  & $(6,3)$ & \textbf{C} & $4$  & $\mathbf{2.00}$\\
	\textbf{2}  & $(8,5)$ & \textbf{B} & $4$  & $\mathbf{2.00}$\\
	\textbf{3}  & $(7,7)$ & \textbf{B} & $5$  & $\mathbf{2.24}$\\
	\textbf{4}  & $(8,7)$ & \textbf{B} & $8$  & $\mathbf{2.83}$\\
	\textbf{5}  & $(5,2)$ & \textbf{C} & $10$ & $\mathbf{3.16}$\\
	\textbf{6}  & $(9,6)$ & \textbf{B} & $10$ & $\mathbf{3.16}$\\
	\textbf{7}  & $(3,7)$ & \textbf{A} & $13$ & $\mathbf{3.61}$\\
	8  & $(2,5)$ & A & $16$ & $4.00$\\
	9  & $(6,1)$ & C & $16$ & $4.00$\\
	10 & $(3,2)$ & C & $18$ & $4.24$\\
	11 & $(9,8)$ & B & $18$ & $4.24$\\
	...
	% axes
	\draw[->,thick] (0,0)--(10.6,0) node[right]{$x_1$};
	\draw[->,thick] (0,0)--(0,9.6) node[above]{$x_2$};
	% cluster clouds (dashed, filled)
	\draw[cnavy,dashed,thick,fill=cnavy!10]  (1.9,6.6) ellipse [x radius=1.7, y radius=1.9];
	\draw[cteal,dashed,thick,fill=cteal!12]  (8.2,6.6) ellipse [x radius=1.6, y radius=1.9];
	\draw[ccoral,dashed,thick,fill=ccoral!10](4.7,1.8) ellipse [x radius=2.2, y radius=1.5];
	% class labels
	\node[cnavy]        at (1.9,9.0) {\textbf{Class A}};
	\node[cteal!70!black] at (8.2,9.0) {\textbf{Class B}};
	\node[ccoral]       at (8.4,1.8) {\textbf{Class C}};
	% Class A points (blue circles)
	\foreach \p in {(1,6),(2,7),(2,5),(3,7),(1,8)}
	  \node[circle,fill=cnavy,minimum size=4.5pt,inner sep=0pt] at \p {};
	% Class B points (teal squares)
	\foreach \p in {(8,7),(9,6),(9,8),(8,5),(7,7)}
	  \node[fill=cteal,minimum size=4pt,inner sep=0pt] at \p {};
	% Class C points (coral diamonds)
	\foreach \p in {(4,1),(5,2),(6,1),(3,2),(6,3)}
	  \node[fill=ccoral,minimum size=4pt,inner sep=0pt,rotate=45] at \p {};
	% arrows from P to the nearest point of each class
	\draw[-{Latex[length=2.5mm]},thick] (6,5)--(3.15,6.9);
	\draw[-{Latex[length=2.5mm]},thick] (6,5)--(7.85,5.05);
	\draw[-{Latex[length=2.5mm]},thick] (6,5)--(6,3.2);
	% query P
	\node[circle,fill=corange,draw=black,minimum size=7pt,inner sep=0pt] at (6,5) {};
	\node[font=\bfseries] at (5.55,5.4) {P};
	\end{tikzpicture}
