...
$\pd{z^{l+1}_{i}}{z^{l}_{j}}$. Since
$z^{l+1}_{i}=\sum_{j}w^{l+1}_{ij}\,\varphi(z^{l}_{j})+b^{l+1}_{i}$, only the term
containing $z^{l}_{j}$ survives and $\varphi$ differentiates to $\varphi'$, so
\[
