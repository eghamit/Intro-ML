...
\[
a=\sigma\!\Bigl(\sum_k w_k x_k + b\Bigr).
\]
...
\node[draw,thick,minimum size=11mm] (act) at (5.8,0){$\sigma$};
\node (out) at (8.4,0){$a=\sigma(z)$};
...
\node[circle,draw,fill=gray!12,minimum size=7mm] (b0) at (0,-3){$1$};
\draw[->] (b0)--(sum) node[pos=0.62,below,sloped]{\scriptsize $b$};
...
activation $\sigma$ to give the output $a$.}
\label{fig:neuron}
\end{figure}
...
% bias units (constant 1) feeding each hidden and output neuron
\node[neuron,fill=gray!12,minimum size=7mm] (bh) at (1.5,-3){$1$};
\node[neuron,fill=gray!12,minimum size=7mm] (bo) at (4.5,-3){$1$};
\draw[->,gray] (bh)--(h1) node[pos=0.5,left,font=\scriptsize,text=black]{$b_{h_1}$};
\draw[->,gray] (bh)--(h2) node[pos=0.5,below,font=\scriptsize,text=black]{$b_{h_2}$};
\draw[->,gray] (bo)--(o)  node[pos=0.5,right,font=\scriptsize,text=black]{$b_o$};
\end{tikzpicture}
\caption{A fully connected $2$--$2$--$1$ multilayer perceptron. Each edge carries
a weight; the grey $1$ nodes are \emph{bias units} that feed a bias $b$ into
every hidden and output neuron.}
\label{fig:mlp}
\end{figure}
...
\begin{itemize}[nosep]
  \item $w^{l}_{jk}$ --- weight from neuron $k$ in layer $l-1$ to neuron $j$ in
        layer $l$;
  \item $b^{l}_{j}$ --- bias of neuron $j$ in layer $l$;
  \item $z^{l}_{j}=\sum_k w^{l}_{jk}\,a^{l-1}_{k}+b^{l}_{j}$ --- the
        \emph{net input} (pre-activation);
  \item $a^{l}_{j}=\sigma\!\left(z^{l}_{j}\right)$ --- the \emph{activation};
        with $a^{0}_{k}=x_k$ (the inputs).
\end{itemize}
In vector form, with weight matrix $\W^{l}$ and bias vector $\vb^{l}$,
\begin{equation}
\vz^{l}=\W^{l}\va^{l-1}+\vb^{l},\qquad \va^{l}=\sigma(\vz^{l}).
\label{eq:forward}
\end{equation}
...
differentiation. For classification one uses \emph{cross-entropy} instead.
...
through its own activation $a^{L}_{j}=\sigma(z^{L}_{j})$:
\begin{equation}
\delta^{L}_{j}=\pd{E}{z^{L}_{j}}
=\pd{E}{a^{L}_{j}}\,\pd{a^{L}_{j}}{z^{L}_{j}}
=\pd{E}{a^{L}_{j}}\,\sigma'(z^{L}_{j}).
\label{eq:deltaL}
\end{equation}
For the MSE loss \eqref{eq:mse}, $\partial E/\partial a^{L}_{j}=a^{L}_{j}-y_j$,
so
\begin{equation}
\boxed{\ \delta^{L}_{j}=\bigl(a^{L}_{j}-y_j\bigr)\,\sigma'(z^{L}_{j})\ }
\label{eq:deltaLmse}
\end{equation}
...
\label{eq:hid1}
\end{equation}
Now $z^{l+1}_{i}=\sum_{j}w^{l+1}_{ij}\,\sigma(z^{l}_{j})+b^{l+1}_{i}$, so
\[
\pd{z^{l+1}_{i}}{z^{l}_{j}}=w^{l+1}_{ij}\,\sigma'(z^{l}_{j}).
\]
Substituting,
\begin{equation}
\boxed{\ \delta^{l}_{j}=\Bigl(\sum_{i}w^{l+1}_{ij}\,\delta^{l+1}_{i}\Bigr)\,\sigma'(z^{l}_{j})\ }
\label{eq:hid2}
\end{equation}
...
$\delta_{h_1}=w_5\delta_o\,\sigma'(z_{h_1})$ and
$\delta_{h_2}=w_6\delta_o\,\sigma'(z_{h_2})$---equation \eqref{eq:hid2}.}
\label{fig:backflow}
\end{figure}
...
\begin{equation}
\vd^{L}=\nabla_{\va}E\odot\sigma'(\vz^{L}),\qquad
\vd^{l}=\bigl((\W^{l+1})^{\!\top}\vd^{l+1}\bigr)\odot\sigma'(\vz^{l}),
\label{eq:vecdelta}
\end{equation}
\begin{equation}
...
\begin{enumerate}[nosep,leftmargin=2.4em]
  \item \textbf{Forward pass.} Set $\va^{0}=\vx$; for $l=1,\dots,L$ compute
        $\vz^{l}=\W^{l}\va^{l-1}+\vb^{l}$ and $\va^{l}=\sigma(\vz^{l})$; store
        all $\vz^{l},\va^{l}$.
  \item \textbf{Output error.} $\vd^{L}=(\va^{L}-\vy)\odot\sigma'(\vz^{L})$.
...
Activation & sigmoid & Loss $E=\tfrac12(o-t)^2$,\ \ $\eta=0.5$\\
\bottomrule
\end{tabular}
\end{center}
...
$w^{l}_{jk}$ & weight from neuron $k$ (layer $l-1$) to neuron $j$ (layer $l$)\\
$b^{l}_{j}$ & bias of neuron $j$ in layer $l$\\
$z^{l}_{j}$ & net input (pre-activation) of neuron $j$ in layer $l$\\
$a^{l}_{j}$ & activation $\sigma(z^{l}_{j})$; $a^{0}=x$ (inputs)\\
$\sigma,\sigma'$ & activation function and its derivative\\
$E$ & loss (error)\\
$\delta^{l}_{j}=\partial E/\partial z^{l}_{j}$ & error signal of neuron $j$ in layer $l$\\
