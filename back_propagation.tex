...
\paragraph{The goal.} Recall the error signal of a neuron,
\[
\delta^{l}_{j}=\pd{E}{z^{l}_{j}},
\]
which is how much the loss changes if we nudge neuron $j$'s net input. For the
...
$\partial z^{l+1}_{i}/\partial z^{l}_{j}$. Recall the next layer's net input:
\[
z^{l+1}_{i}=\sum_{k}w^{l+1}_{ik}\,\varphi(z^{l}_{k})+b^{l+1}_{i}
\]
(the summation index is renamed to $k$ to avoid clashing with the fixed $j$). It
depends on $z^{l}_{j}$ only through the activation $\varphi(z^{l}_{j})$, so we
differentiate term by term and apply the chain rule to each $\varphi(z^{l}_{k})$:
\begin{align}
\pd{z^{l+1}_{i}}{z^{l}_{j}}
