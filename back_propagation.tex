...
\paragraph{The goal.} Recall the error signal $\delta^{l}_{j}=\pd{E}{z^{l}_{j}}$
--- how much the loss changes if we nudge neuron $j$'s net input. For the
\emph{output} layer this was immediate (equation~\eqref{eq:deltaLmse}); for a
\emph{hidden} neuron, which never feeds the loss directly, we must work a little
harder.

\paragraph{The key idea: a hidden neuron affects the loss only through the next
layer.} A hidden neuron $j$ in layer $l$ sends its activation
$a^{l}_{j}=\varphi(z^{l}_{j})$ to \emph{all} neurons $i$ of the next layer $l+1$;
it has no other route to the loss. So the multivariate chain rule adds up its
effect through every one of them:
\begin{equation}
\delta^{l}_{j}=\pd{E}{z^{l}_{j}}
=\sum_{i}\underbrace{\pd{E}{z^{l+1}_{i}}}_{=\,\delta^{l+1}_{i}}\,
   \pd{z^{l+1}_{i}}{z^{l}_{j}}
=\sum_{i}\delta^{l+1}_{i}\,\pd{z^{l+1}_{i}}{z^{l}_{j}}.
\label{eq:hid1}
\end{equation}
The first factor $\pd{E}{z^{l+1}_{i}}$ is exactly $\delta^{l+1}_{i}$---an error
signal we \emph{already computed} for the next layer, which is the reuse that
makes backpropagation efficient.

\paragraph{Working out the remaining piece.} We still need
$\partial z^{l+1}_{i}/\partial z^{l}_{j}$. Recall that the next-layer net input
is $z^{l+1}_{i}=\sum_{k}w^{l+1}_{ik}\,\varphi(z^{l}_{k})+b^{l+1}_{i}$ (we rename
the summation index to $k$ to avoid clashing with our fixed $j$). It depends on
$z^{l}_{j}$ only through the activation $\varphi(z^{l}_{j})$, so we differentiate
term by term and apply the chain rule to each $\varphi(z^{l}_{k})$:
\begin{align}
\pd{z^{l+1}_{i}}{z^{l}_{j}}
&=\pd{}{z^{l}_{j}}\!\left[\sum_{k}w^{l+1}_{ik}\,\varphi(z^{l}_{k})+b^{l+1}_{i}\right]
 =\sum_{k}w^{l+1}_{ik}\,\varphi'(z^{l}_{k})\,\pd{z^{l}_{k}}{z^{l}_{j}}
 \notag\\[2pt]
&=w^{l+1}_{ij}\,\varphi'(z^{l}_{j}).
\label{eq:dzdz}
\end{align}
Every term in the sum vanishes except the one with $k=j$, because the
pre-activations of a layer are independent variables: $\partial z^{l}_{k}/\partial
z^{l}_{j}=1$ when $k=j$ and $0$ otherwise. The bias $b^{l+1}_{i}$ is constant in
$z^{l}_{j}$ and drops out. What survives is a single product---the forward weight
$w^{l+1}_{ij}$ times this neuron's activation slope $\varphi'(z^{l}_{j})$.
\paragraph{The result (the backward recursion).} Substituting,
\begin{equation}
...
\paragraph{How to read it.} The formula has two factors, each with a plain
meaning:
\begin{itemize}[nosep]
  \item $\displaystyle\sum_{i}w^{l+1}_{ij}\,\delta^{l+1}_{i}$ --- gather the error
        signals of the neurons this neuron feeds, each weighted by the very
        connection $w^{l+1}_{ij}$ it sent forward. A neuron that pushed hard (a
        large weight) on a badly-wrong neuron (a large $\delta$) inherits more of
        the blame.
  \item $\varphi'(z^{l}_{j})$ --- scale by this neuron's own activation slope. A
        saturated neuron ($\varphi'\approx0$) can barely change the output, so
        its error signal is tiny.
\end{itemize}
\paragraph{Why it is a recursion.} The formula writes $\delta^{l}$ in terms of
$\delta^{l+1}$, one layer nearer
the output. So we compute the output signal first
(equation~\eqref{eq:deltaLmse}) and apply \eqref{eq:hid2} repeatedly to get
$\delta^{L-1},\delta^{L-2},\dots$---marching \emph{backward} to the first layer.
That backward march is backpropagation; the rule \eqref{eq:gradw} then turns each
$\delta$ into a weight gradient.

\begin{example}[Computing one hidden error signal]
A hidden neuron with sigmoid activation output $a=0.5$ (so its slope is
$\varphi'=a(1-a)=0.25$) feeds two output neurons through weights $0.6$ and
$-0.4$, whose error signals are $\delta_{1}=\delta_{2}=0.5$. Then
\[
\delta=\bigl(0.6(0.5)+(-0.4)(0.5)\bigr)\times 0.25
      =(0.30-0.20)\times 0.25=0.10\times 0.25=0.025 .
\]
The weighted sum of downstream errors is $0.10$; the activation slope $0.25$
shrinks it to the neuron's own error signal $0.025$.
\end{example}
