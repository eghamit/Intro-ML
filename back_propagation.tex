...
a weight; each hidden and output neuron also has a bias (not drawn).}
\label{fig:mlp}
\end{figure}
...
\centering
\begin{tikzpicture}
\begin{axis}[width=0.72\textwidth,height=6cm,axis lines=middle,
  xlabel={$z$},xmin=-6,xmax=6,ymin=-1.4,ymax=2.1,
  legend pos=north west,legend cell align=left,
  grid=both,major grid style={gray!20},samples=101]
\addplot[thick,cin,domain=-6:6]{1/(1+exp(-x))};   \addlegendentry{sigmoid}
\addplot[thick,chid,domain=-6:6]{tanh(x)};         \addlegendentry{tanh}
\addplot[thick,cout,domain=-6:2]{max(0,x)};        \addlegendentry{ReLU}
\end{axis}
\end{tikzpicture}
\caption{The three most common activation functions. Sigmoid squashes to
$(0,1)$, tanh to $(-1,1)$, and ReLU passes positive values unchanged.}
\label{fig:acts}
\end{figure}
