...
Figure~\ref{fig:anatomy} labels these four ingredients on a small example.
...
\begin{tikzpicture}[
   ...
   callout/.style={font=\footnotesize,align=left,inner sep=2pt},
   arr/.style={->,thick,gray}]
% ----- leaves -----
\foreach \x/\l in {1/p,2/q,3/r,4/s} \node[below=2pt] at (\x,0){$\l$};
% ----- merge bars (via \merge{x1}{y1}{x2}{y2}{h}) -----
\merge{1}{0}{2}{0}{2}       % p,q merge at height 2  (centre x=1.5)
\merge{3}{0}{4}{0}{3}       % r,s merge at height 3  (centre x=3.5)
\merge{1.5}{2}{3.5}{3}{5}   % root merge at height 5 (centre x=2.5)
% mark the internal nodes
\node[dot] at (1.5,2){};
\node[dot] at (3.5,3){};
\node[dot] at (2.5,5){};
% ----- height axis with dashed guides -----
\draw[->] (-0.4,0)--(-0.4,5.8) node[above]{\scriptsize height};
\foreach \y in {2,3,5} \draw (-0.5,\y)--(-0.3,\y) node[left]{\scriptsize \y};
\draw[gray!55,dashed] (-0.3,2)--(1.5,2);
\draw[gray!55,dashed] (-0.3,3)--(3.5,3);
\draw[gray!55,dashed] (-0.3,5)--(2.5,5);
% ----- callouts -----
\node[callout] (cLeaf) at (6.1,0){\textbf{1. Single object}\\a \emph{leaf} of the tree};
\draw[arr] (cLeaf.west) to[bend left=8] (4.05,-0.15);
\node[callout] (cNode) at (6.1,2){\textbf{2. Internal node}\\a merge of its two children};
\draw[arr] (cNode.west) to[bend right=6] (1.6,2);
\node[callout] (cClus) at (6.1,3.3){\textbf{3. Grouped cluster}\\$\{r,s\}$ formed by this merge};
\draw[arr] (cClus.west) to[bend left=6] (3.55,3.03);
\node[callout] (cH) at (6.1,5){\textbf{4. Height}\\$=$ distance at which\\the merge occurs};
\draw[arr] (cH.west) to[bend left=6] (2.55,5);
\end{tikzpicture}
\caption{Anatomy of a dendrogram. The leaves $p,q,r,s$ are the
\textbf{single objects}; each horizontal bar is an \textbf{internal node} that
merges its two children into one \textbf{grouped cluster}; the vertical level of
that bar---read off the left axis via the dashed guides---is its
\textbf{height}, the distance at which the merge happened.}
\label{fig:anatomy}
\end{figure}
...
$(CE)$} into $(ACE)$ at height $3$. The update rule is the same single-linkage
minimum as before, but now one of the two things being merged is \emph{itself a
cluster}, $(CE)$. So for any remaining cluster $X$,
\[
d\bigl((ACE),X\bigr)=\min\bigl\{\,d(A,X),\ d((CE),X)\,\bigr\},
\]
i.e.\ we take, column by column, the smaller of row $A$ and row $(CE)$ from
$\mathbf D^{(1)}$. Substituting the two remaining columns $X=B$ and $X=D$:
\[
\begin{aligned}
d\bigl((ACE),B\bigr)&=\min\bigl\{\,d(A,B),\ d((CE),B)\,\bigr\}=\min\{9,\,7\}=7,\\[2pt]
d\bigl((ACE),D\bigr)&=\min\bigl\{\,d(A,D),\ d((CE),D)\,\bigr\}=\min\{6,\,8\}=6.
\end{aligned}
\]
The values $9,7$ and $6,8$ are read straight from rows $A$ and $(CE)$ of
$\mathbf D^{(1)}$. Rows/columns $A$ and $(CE)$ are then deleted and replaced by
the single new row/column $(ACE)$:
\begin{equation}
\mathbf{D}^{(2)}=
\begin{array}{c|ccc}
     & B & D & (ACE)\\ \hline
...
For a larger practice problem we cluster \textbf{six} objects
$P_1,\dots,P_6$ using \textbf{complete linkage}, where the distance between two
clusters is the \emph{maximum} of the pairwise distances. At each step we merge
the closest pair and update the matrix by element-wise \emph{maxima}. The given
proximity matrix is
\begin{equation}
\mathbf{D}^{(0)}=
...
\section{Step 1}
The smallest off-diagonal entry is $2$, attained by both $P_1P_2$ and $P_5P_6$;
we take $P_1,P_2$ first. \textbf{Merge $P_1,P_2$} at height $2$ and update by
maxima, $d((P_1P_2),X)=\max\{d(P_1,X),d(P_2,X)\}$:
\[
d((P_1P_2),P_3)=\max(6,5)=6,\quad d((P_1P_2),P_4)=\max(10,9)=10,\quad
d((P_1P_2),P_5)=\max(9,8)=9,\quad d((P_1P_2),P_6)=\max(8,7)=8.
\]
\begin{equation}
\mathbf{D}^{(1)}=
\begin{array}{c|ccccc}
       & (P_1P_2) & P_3 & P_4 & P_5 & P_6\\ \hline
(P_1P_2) & 0  & 6 & 10 & 9 & 8\\
P_3    & 6  & 0 & 4  & 5 & 6\\
P_4    & 10 & 4 & 0  & 3 & 5\\
P_5    & 9  & 5 & 3  & 0 & 2\\
...
\section{Step 2}
The smallest remaining entry is $2$ ($P_5P_6$). \textbf{Merge $P_5,P_6$} at
height $2$:
\[
d((P_5P_6),(P_1P_2))=\max(9,8)=9,\quad d((P_5P_6),P_3)=\max(5,6)=6,\quad
d((P_5P_6),P_4)=\max(3,5)=5.
\]
\begin{equation}
\mathbf{D}^{(2)}=
\begin{array}{c|cccc}
       & (P_1P_2) & P_3 & P_4 & (P_5P_6)\\ \hline
(P_1P_2) & 0  & 6 & 10 & 9\\
P_3    & 6  & 0 & 4  & 6\\
P_4    & 10 & 4 & 0  & 5\\
(P_5P_6) & 9  & 6 & 5  & 0
...
\section{Step 3}
The smallest entry is $d(P_3,P_4)=4$. \textbf{Merge $P_3,P_4$} at height $4$:
\[
d((P_3P_4),(P_1P_2))=\max(6,10)=10,\qquad d((P_3P_4),(P_5P_6))=\max(6,5)=6.
\]
\begin{equation}
\mathbf{D}^{(3)}=
\begin{array}{c|ccc}
        & (P_1P_2) & (P_3P_4) & (P_5P_6)\\ \hline
(P_1P_2)  & 0  & 10 & 9\\
(P_3P_4)  & 10 & 0  & 6\\
(P_5P_6)  & 9  & 6  & 0
\end{array}
...
\section{Step 4}
The smallest entry is $d((P_3P_4),(P_5P_6))=6$. \textbf{Merge} them at height $6$
into $(P_3P_4P_5P_6)$:
\[
d\bigl((P_3P_4P_5P_6),(P_1P_2)\bigr)=\max(10,9)=10.
\]
\begin{equation}
\mathbf{D}^{(4)}=
\begin{array}{c|cc}
             & (P_1P_2) & (P_3P_4P_5P_6)\\ \hline
(P_1P_2)       & 0  & 10\\
(P_3P_4P_5P_6) & 10 & 0
\end{array}
\end{equation}
...
\section{Step 5}
Only two clusters remain; merge $(P_1P_2)$ and $(P_3P_4P_5P_6)$ at height $10$
into the root $\{P_1,\dots,P_6\}$.
...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[yscale=0.42,xscale=1.05]
\foreach \x/\l in {1/{P_1},2/{P_2},3/{P_3},4/{P_4},5/{P_5},6/{P_6}}
  \node[below] at (\x,0){$\l$};
\merge{1}{0}{2}{0}{2}          % P1,P2 at 2  -> x=1.5
\merge{5}{0}{6}{0}{2}          % P5,P6 at 2  -> x=5.5
\merge{3}{0}{4}{0}{4}          % P3,P4 at 4  -> x=3.5
\merge{3.5}{4}{5.5}{2}{6}      % (P3P4),(P5P6) at 6 -> x=4.5
\merge{1.5}{2}{4.5}{6}{10}     % (P1P2),(P3P4P5P6) at 10
\draw[->] (0.2,0)--(0.2,11.5) node[above]{\scriptsize height};
\foreach \y in {2,4,6,10} \draw (0.1,\y)--(0.3,\y) node[left]{\scriptsize \y};
\end{tikzpicture}
\caption{Complete-linkage dendrogram for the six objects. The two tight pairs
$\{P_1,P_2\}$ and $\{P_5,P_6\}$ form first (height $2$), then $\{P_3,P_4\}$,
which joins $\{P_5,P_6\}$; the far-apart group $\{P_1,P_2\}$ merges last at the
large height $10$.}
\label{fig:compdend}
\end{figure}
...
\end{tabular}
\end{center}
Average linkage attaches $A$ to $(CE)$ only at step~3 (as single linkage does),
yet merges $B,D$ earlier at step~2---illustrating how average linkage sits
``between'' the single- and complete-linkage extremes.
...
three clusters obtained by cutting just below height $6$.\\
\textbf{Solution.} The merge of $(P_3P_4)$ with $(P_5P_6)$ happens \emph{at}
height $6$ (and the root at $10$); cutting just below $6$ severs both. The
remaining clusters are $\{P_1,P_2\}$, $\{P_3,P_4\}$ and $\{P_5,P_6\}$ --- three
...
Given the single-linkage merge heights $2,3,5,6$ (Chapter~\ref{ch:single}) and
the complete-linkage heights $2,2,4,6,10$ (Chapter~\ref{ch:complete}), use the
``largest gap'' rule to recommend $k$ for each and comment on the difference.
\end{exercise}
