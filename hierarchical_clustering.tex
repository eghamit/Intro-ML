...
describing it. Let $\vx$ and $\vy$ be \emph{two such points} (two different
objects). Being a point of $\R^n$, each one carries $n$ coordinates---one per
feature---which we write with a subscript:
\[
\vx=(x_1,x_2,\dots,x_n),\qquad \vy=(y_1,y_2,\dots,y_n).
\]
Here $x_k$ is the $k$-th coordinate of the \emph{same} point $\vx$; it is
\textbf{not} a separate point. In the familiar plane ($n=2$) this is just the
usual abscissa/ordinate pair: $\vx=(x_1,x_2)$ is one point and
$\vy=(y_1,y_2)$ is another, and the distance formulas below reduce to the
schoolbook $\sqrt{(x_1-y_1)^2+(x_2-y_2)^2}$. For $n>2$ we simply have more
...
\[
d(\vx,\vy)=1-\frac{\vx^{\!\top}\vy}{\lVert\vx\rVert\,\lVert\vy\rVert},
\]
...
$d(\vx,\vy)=d(\vy,\vx)$, identity $d(\vx,\vx)=0$, and the triangle inequality.
...
cluster:
\begin{equation}
d_{\text{single}}(\C_i,\C_j)=\min_{\vx\in\C_i,\ \vy\in\C_j} d(\vx,\vy).
\label{eq:single}
\end{equation}
...
\begin{equation}
d_{\text{complete}}(\C_i,\C_j)=\max_{\vx\in\C_i,\ \vy\in\C_j} d(\vx,\vy).
\label{eq:complete}
\end{equation}
...
d_{\text{average}}(\C_i,\C_j)=\frac{1}{|\C_i|\,|\C_j|}
   \sum_{\vx\in\C_i}\sum_{\vy\in\C_j} d(\vx,\vy).
\label{eq:average}
\end{equation}
